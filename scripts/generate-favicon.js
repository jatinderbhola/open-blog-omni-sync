...
async function generateFavicon() {
	...
	const favicon16 = await sharp(Buffer.from(svg)).resize(16, 16).png().toBuffer();

	const favicon32 = await sharp(Buffer.from(svg)).resize(32, 32).png().toBuffer();

	// Use the 32x32 version as favicon.ico
	await fs.writeFile(path.join(process.cwd(), 'public', 'favicon.ico'), favicon32);
